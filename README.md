...
- Compile protobufs for frontend (assumes working directory is the directory `frontend`)

  ```
    # Install `protoc-gen-connect-es` tool from npm
    npm install -D @connectrpc/protoc-gen-connect-es --legacy-peer-deps

    # Generate auth proto files
    mkdir -p src/proto/auth && \
    npx protoc -I . \
        --es_out src/proto/auth \
        --es_opt target=js \
        --connect-es_out src/proto/auth \
        --connect-es_opt target=js \
        auth.proto

    # Generate chat proto files
    mkdir -p src/proto/chat && \
    npx protoc -I . \
        --es_out src/proto/chat \
        --es_opt target=js \
        --connect-es_out src/proto/chat \
        --connect-es_opt target=js \
        chat.proto
  ```
